...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        def _debug_print_and_shapes(batch_lst, concat_dim=0):
            print("=== DataProto.concat detailed debug ===")
            max_shapes = {}
            # Determine keys if tensordict-like
            for idx, b in enumerate(batch_lst):
                if b is None:
                    print(f"  batch[{idx}] = None")
                    continue
                # tensordict or dict-like
                try:
                    keys = list(b.keys())
                except Exception:
                    # fallback: treat b as tensor
                    try:
                        print(f"  batch[{idx}].shape = {tuple(b.shape)}")
                    except Exception:
                        print(f"  batch[{idx}] type = {type(b)}")
                    continue

                print(f"  batch[{idx}] keys: {keys}")
                for k in keys:
                    try:
                        t = b[k]
                        if t is None:
                            print(f"    [{k}] = None")
                            continue
                        # some entries may be lists/tuples
                        if isinstance(t, (list, tuple)):
                            print(f"    [{k}] is list/tuple len={len(t)}")
                            continue
                        if hasattr(t, "shape"):
                            s = tuple(t.shape)
                            print(f"    [{k}].shape = {s}")
                            # track max along each dim across items (for this key)
                            prev = max_shapes.get(k)
                            if prev is None:
                                max_shapes[k] = list(s)
                            else:
                                # extend to longer dims
                                if len(prev) < len(s):
                                    prev.extend([0] * (len(s) - len(prev)))
                                for di, sv in enumerate(s):
                                    if di >= len(prev):
                                        prev.append(sv)
                                    else:
                                        prev[di] = max(prev[di], sv)
                        else:
                            print(f"    [{k}] type = {type(t)}")
                    except Exception as e:
                        print(f"    [{k}] -> exception {e}")
            print("  Computed max_shapes per key:")
            for k, v in max_shapes.items():
                print(f"    key={k} max_shape={tuple(v)}")
            sys.stdout.flush()
            return max_shapes

        # Call debug function (this will print shapes into stdout/slurm logs)
        # max_shapes = _debug_print_and_shapes(batch_lst, concat_dim=0)
        # --- DEBUG PATCH END ---
