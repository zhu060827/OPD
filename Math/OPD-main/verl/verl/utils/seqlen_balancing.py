...
def get_reverse_idx(idx_map):
    ...
    Args:
        idx_map (Sequence[int]): Sequence where idx_map[i] = j, meaning original position i
                                 is mapped to new position j. All values in idx_map must be
                                 in range [0, len(idx_map)) and form a valid permutation.

    Returns:
        List[int]: Inverse mapping list such that output[j] = i for each i.
                  The length of output equals len(idx_map).
    """
    n = len(idx_map)
    if n == 0:
        return []
    ...
    min_idx = min(idx_map) if idx_map else -1
    if max_idx >= n or min_idx < 0:
        raise ValueError(
            f"Invalid index mapping: indices must be in range [0, {n}), "
            f"but got min={min_idx}, max={max_idx}"
        )
    
    # Initialize reverse mapping with -1 to detect missing indices
    reverse_idx_map = [-1] * n
    
    # Build reverse mapping: reverse_idx_map[j] = i where idx_map[i] = j
    for i, idx in enumerate(idx_map):
        if reverse_idx_map[idx] != -1:
            raise ValueError(
                f"Invalid index mapping: duplicate index {idx} found at positions "
                f"{reverse_idx_map[idx]} and {i}"
            )
        reverse_idx_map[idx] = i
    
    # Verify all positions are filled (ensures it's a valid permutation)
    if -1 in reverse_idx_map:
        missing = [i for i, val in enumerate(reverse_idx_map) if val == -1]
        raise ValueError(
            f"Invalid index mapping: missing indices in reverse mapping: {missing}"
        )
